...
> `../docs/` (in particular `architecture/`, `security/`, `operations/`) before changing anything,
> and keep that documentation up to date whenever behavior, schema or security properties change.
...
1. **Always Include Tenant Isolation:** Every new business table must include `office_id UUID NOT NULL` (or equivalent tenant key) and foreign key constraints unless explicitly global.
2. **Enable Row Level Security (RLS):** New tables storing sensitive or tenant-specific data must include `ALTER TABLE {table_name} ENABLE ROW LEVEL SECURITY;` and its corresponding tenant policy in the migration script. Policies must compare `office_id` against `current_setting('app.current_office_id', true)`. The transaction layer MUST execute `set_config('app.current_office_id', '<office-id>', false)` (equivalent to `SET LOCAL`) on the current JDBC connection before any query.
...
* Never use `SET` (session-level) for the office context: it leaks across pooled connections. Only `SET LOCAL` (via `set_config(..., false)`) inside the current transaction.
...
1. **RBAC Rules:**
   * Roles and permissions are **database-managed and fully mutable**: the seeded office roles
     `MEDICO_TITOLARE`, `MEDICO_COLLABORATORE`, `SEGRETARIA_BASE` and `SEGRETARIA_AVANZATA`, plus
     the global platform role `ADMIN`, are seed data, not hard-coded authorization branches.
   * Multiple role assignments aggregate permissions by set union (logical OR).
   * Authorization must be resolved from the current database state on every request
     (`PermissionResolver`); JWT role/permission claims are informational only.
   * `ROLE_ADMIN`: system maintenance (global, no office membership required).
   * Office roles: access to clinical notes, prescription issuing and patient data is limited to
     the permissions granted by the user's active office roles within their office.
...
3. **Audit Log Mandatory Metadata:**
   * Every audit log entry must record: `operator_id`, `action_type`, `patient_id`, `office_id`, `timestamp`, `ip_address`.
...
* **Multi-Tenancy Checks:** Ensure Spring Data JPA specifications or PostgreSQL RLS filters explicitly enforce `office_id` context on queries.

...
public class PrescriptionService {
  ...
  public List<PrescriptionResponse> list(UUID officeId) {
    ...
    auditService.record(
        AuditActionType.PRESCRIPTION_READ, "PRESCRIPTION", null, null, officeId, null);
  ...
  public PrescriptionResponse request(UUID officeId, RequestPrescriptionRequest request) {
    ...
    prescription.setMedication(request.medication().trim());
  ...
  public PrescriptionResponse issue(UUID officeId, UUID prescriptionId) {
    ...
    // Atomic transition so concurrent issue requests produce exactly one winner.
    if (prescriptionRepository.issueIfRequested(prescriptionId, Instant.now()) == 0) {
  ...
  public PrescriptionResponse print(UUID officeId, UUID prescriptionId) {
    ...
    officeGuard.requireOfficeAccess(officeId);
    Prescription prescription = requirePrescription(prescriptionId);
    // Atomic transition; re-printing an already printed prescription is allowed and idempotent.
    if (prescriptionRepository.printIfIssued(prescriptionId, Instant.now()) == 0) {
      throw new ApiException(
          ErrorCodes.OPERATION_CONFLICT,
          HttpStatus.CONFLICT,
